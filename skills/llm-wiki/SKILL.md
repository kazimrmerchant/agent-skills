...
- **Wiki path:** Set via `WIKI_PATH` environment variable. On Windows, set it in PowerShell:
  ```powershell
  [Environment]::SetEnvironmentVariable("WIKI_PATH", "~\wiki", "User")
  $env:WIKI_PATH = "~\wiki"
  ```
  If unset, defaults to `~/wiki` (on Windows: `$HOME\wiki`, typically `~\wiki`).

- **No database or special tooling required** — the wiki is just a directory of markdown files.
  Open it in Obsidian, VS Code, or any editor.

- **Optional — Obsidian integration:** If the user wants Obsidian graph view, Dataview, or sync,
  set `OBSIDIAN_VAULT_PATH` to the same directory as the wiki path. See the Obsidian Integration
  section below.

- **Optional — Obsidian headless (servers):** Requires Node.js 22+ and an Obsidian Sync
  subscription. See the Obsidian Headless section below.
...
```powershell
# PowerShell — resolve wiki path
$WIKI = if ($env:WIKI_PATH) { $env:WIKI_PATH } else { "$HOME\wiki" }
```

```bash
# Bash (Linux/macOS) — resolve wiki path
...
```powershell
# PowerShell — orientation reads at session start
$WIKI = if ($env:WIKI_PATH) { $env:WIKI_PATH } else { "$HOME\wiki" }
...
2. Create the directory structure shown in the Architecture section above.
   ```powershell
   $WIKI = if ($env:WIKI_PATH) { $env:WIKI_PATH } else { "$HOME\wiki" }
   New-Item -ItemType Directory -Force -Path "$WIKI\raw\articles", "$WIKI\raw\papers", "$WIKI\raw\transcripts", "$WIKI\raw\assets", "$WIKI\entities", "$WIKI\concepts", "$WIKI\comparisons", "$WIKI\queries"
   ```
...
3. **Check what already exists** — search `index.md` and use `search_files` to find
...
### Core Operation 3 — Lint

When the user asks to lint, health-check, or audit the wiki:

1. **Orphan pages:** Find pages with no inbound `[[wikilinks]]` from other pages.
   Use `execute_code` for a programmatic scan across all wiki pages:
   ```python
   import os, re
...
8. **Source drift:** For each file in `raw/` with a `sha256:` frontmatter, recompute
...
```powershell
# PowerShell — find pages by content
...
# Recent activity — read last 20 lines of log
# read_file "$WIKI\log.md" (tail 20)
```
...
**Continuous background sync via systemd (Linux):**
...
- **NEVER modify files in `raw/`** — sources are immutable. Corrections go in wiki pages. This is a hard rule.
...
- **Windows path separators** — use backslashes in PowerShell commands (`$WIKI\SCHEMA.md`), forward slashes in cross-platform markdown links (`raw/articles/source.md`).

## Verification

After any wiki operation, verify the results:

```powershell
# PowerShell — verify wiki structure exists
$WIKI = if ($env:WIKI_PATH) { $env:WIKI_PATH } else { "$HOME\wiki" }
Test-Path "$WIKI\SCHEMA.md"
Test-Path "$WIKI\index.md"
Test-Path "$WIKI\log.md"
Test-Path "$WIKI\raw"
Test-Path "$WIKI\entities"
Test-Path "$WIKI\concepts"
```

**Expected output:** All commands return `True`.

```powershell
# Verify index page count matches filesystem
$wikiPages = Get-ChildItem -Path "$WIKI\entities","$WIKI\concepts","$WIKI\comparisons","$WIKI\queries" -Filter "*.md" -Recurse -ErrorAction SilentlyContinue
$wikiPages.Count
# Compare this number to the "Total pages: N" line in index.md
```

```powershell
# Verify no raw/ files were modified during the session (immutability check)
# Raw files should only be created, never edited after initial write.
Get-ChildItem -Path "$WIKI\raw" -Filter "*.md" -Recurse | Select-Object FullName, LastWriteTime
```

```powershell
# Verify log.md was appended to (last entry should match today's action)
Get-Content "$WIKI\log.md" -Tail 5
```

**Expected:** The last log entry matches the action just performed (e.g., `## [YYYY-MM-DD] ingest | Source Title`).

```powershell
# Verify all new pages have required frontmatter
# Quick check — read first 10 lines of each new page and confirm YAML frontmatter exists
Get-ChildItem -Path "$WIKI\entities","$WIKI\concepts","$WIKI\comparisons","$WIKI\queries" -Filter "*.md" -Recurse | ForEach-Object {
    $firstLine = Get-Content $_.FullName -TotalCount 1
    if ($firstLine -ne "---") { Write-Warning "Missing frontmatter: $($_.FullName)" }
}
```

**Expected:** No warnings printed. Every wiki page starts with `---`.

## Related Skills

- **obsidian** — Obsidian vault management, Dataview queries, and sync. Set `OBSIDIAN_VAULT_PATH` to the same directory as `WIKI_PATH` for seamless integration.
- **arxiv** — ArXiv paper search and retrieval. Pairs naturally with this skill: use arxiv to find papers, then ingest them into the wiki via `raw/papers/`.
...
use llm-wiki-compiler when you want batch compile of a source directory.
