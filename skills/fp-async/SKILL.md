...
- Familiarity with `Either` and `pipe` from `fp-ts` is helpful but not required — all patterns are self-contained.
- Node.js 18+ recommended for native `fetch` support in examples.
...
- **Fails fast on parallel array operations**: `TE.traverseArray` fails on the first error — all subsequent errors are lost. If you need all errors collected, use a different strategy (e.g., `TE.traverseArray` with ` getApplicativeComposition` or collect errors manually).
- **Race conditions with `sequenceT(TE.ApplyPar)`**: Running dependent operations in parallel causes race conditions. If operation B needs a value from operation A, use `TE.chain` (sequential), not `sequenceT`.
...
- **Error type widening**: If you mix different error types in a pipeline (e.g., `FileError` and `ApiError`), TypeScript will infer a union. This can make error handling harder downstream. Consider a unified error type or use `mapLeft` to normalize.
...
- **Do notation requires `fp-ts` extensions**: `TE.Do` and `TE.bind` are available in `fp-ts` 2.x. Ensure your version supports them.
- **`traverseArray` is on the `TE` module directly**: In some `fp-ts` versions, you may need `TE.traverseArray` (pipeable) vs `A.traverse(TE.ApplicativePar)`. Check your version's API.

## Verification

Verify your setup compiles and patterns work correctly:

1. **Check fp-ts is installed:**
...
3. **Quick smoke test — wrap and execute a TaskEither:**
...
// Verify: result should be { _tag: 'Right', right: 42 }
console.log(E.isRight(result)) // true
console.log(E.isLeft(result))  // false
```
...
// Verify: failResult should be { _tag: 'Left', left: Error('boom') }
console.log(E.isLeft(failResult)) // true
console.log(E.isRight(failResult)) // false
```

5. **Verify parallel execution returns tuple:**
...
// Verify: parResult should be { _tag: 'Right', right: [1, 'hello', true] }
console.log(E.isRight(parResult)) // true
```
...
- **fp-either** — Synchronous `Either` patterns, the foundation of `TaskEither`.
- **fp-option** — `Option` type for nullable values without exceptions.
- **fp-pipe** — `pipe` and flow composition for building readable pipelines.
