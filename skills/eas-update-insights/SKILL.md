...
- Windows host is primary (PowerShell). In PowerShell, replace bash-style `$(...)` command substitution with `$(...)` (same syntax) or assign to a variable first; `jq` must be installed and on `PATH`.
...
### 1. Discover the update group ID
...
```powershell
$GROUP_ID = (eas update:list --branch production --json --non-interactive | jq -r '.currentPage[0].group')
```
...
### 2. Query per-update insights with `eas update:insights <groupId>`
...
JSON output shape: top level `groupId`, `timespan` (`start`, `end`, `daysBack`), and `platforms[]` with one entry per platform the group was published to. Each platform entry has `updateId`, `totals` (`uniqueUsers`, `installs`, `failedInstalls`, `crashRatePercent`), `payload` (`launchAssetCount`, `averageUpdatePayloadBytes`), and a `daily[]` time series of `{ date, installs, failedInstalls }`.

For the complete schema and field reference, load [references/update-insights-schema.md](./references/update-insights-schema.md) when you need to interpret or validate JSON fields from `update:insights` or `update:view --insights`.
...
### 3. View group details with inline insights: `eas update:view <groupId> --insights`
...
### 4. Query channel-level insights: `eas channel:insights --channel <name> --runtime-version <version>`
...
JSON output shape: top level `channel`, `runtimeVersion`, `timespan`, `embeddedUpdateTotalUniqueUsers`, `otaTotalUniqueUsers`, `mostPopularUpdates[]` (each with `rank`, `groupId`, `message`, `platform`, `totalUniqueUsers`), `cumulativeMetricsAtLastTimestamp[]`, plus chart-shaped `uniqueUsersOverTime` and `cumulativeMetricsOverTime` objects with `labels` and `datasets`.

For the complete schema and field reference, load [references/channel-insights-schema.md](./references/channel-insights-schema.md) when you need to interpret or validate JSON fields from `channel:insights`.
...
- **`otaTotalUniqueUsers` undercounts** when more than top-N updates are active, because it is a sum over `mostPopularUpdates[]` only.
- **`--days` / `--start` / `--end` on `update:view`** only apply when `--insights` is set; passing them alone errors.
- **`update:list` without `--branch` or `--all`** will prompt for a branch selection; always pass one of those flags plus `--non-interactive` in scripts.
- **CLI/server version skew**: `EAS Update insights is not supported by this version of eas-cli. Please upgrade ...` means the server deprecated a field the CLI relies on. Run `npm install -g eas-cli@latest`.
- **Wrong platform filter**: `Update group "<id>" has no ios update (available platforms: android)` — `--platform ios` was used but the group wasn't published for iOS.
- **Missing group**: `Could not find any updates with group ID: "<id>"` — group doesn't exist or you lack access.
- **Missing channel**: `Could not find channel with the name <name>` — typo or wrong account.
- **No OTA launches yet**: "No update launches recorded" in the table / empty `mostPopularUpdates` in JSON — no OTA update has been launched for that channel + runtime yet. Usually means the channel is still serving the embedded build only.
- **Display vs JSON naming**: The CLI table labels say "Launches" / "Crashes" while JSON uses `installs` / `failedInstalls`. Same field, different display name.
- **Dates**: `daily[].date` values are UTC ISO timestamps; the human-readable table renders them as `YYYY-MM-DD` (UTC).

## Verification

1. Confirm `eas-cli` is installed and you are logged in:
...
2. Confirm you can list recent updates and extract a group ID:

   ```bash
   eas update:list --branch production --json --non-interactive | jq -r '.currentPage[0].group'
   ```

   Expected: a UUID-like string, e.g. `03d5dfcf-736c-475a-8730-af039c3f4d06`.

3. Confirm insights return for that group:

   ```bash
   eas update:insights 03d5dfcf-736c-475a-8730-af039c3f4d06 --json --non-interactive \
     | jq '.platforms[] | {platform, installs: .totals.installs, crashRate: .totals.crashRatePercent}'
   ```

   Expected: one JSON object per platform with `platform`, `installs`, and `crashRate` fields.

4. Confirm channel insights resolve from an Expo project directory:

   ```bash
   eas channel:insights --channel production --runtime-version 1.0.6 --json --non-interactive \
     | jq '{channel, embedded: .embeddedUpdateTotalUniqueUsers, ota: .otaTotalUniqueUsers}'
   ```

   Expected: JSON with `channel`, `embedded`, and `ota` numeric fields.
...
- `eas-update-publish` — for publishing new update groups before monitoring them here.
- `eas-channels` — for channel and branch configuration that determines which updates are served.
