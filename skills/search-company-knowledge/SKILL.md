...
**Trigger keywords:**
- find information, search company knowledge, look up, what is, explain
- company docs, internal documentation, Confluence search, Jira search
- our documentation, internal knowledge, knowledge base, search for, tell me about
- company systems, terminology, find everything about, what do we know about
- deployment, authentication, infrastructure, processes, procedures
- how to, how does, our systems, our processes, internal systems
- technical documentation, engineering docs, architecture, configuration
- search our docs, search internal docs, find in our docs

### When NOT to Use

This skill is for **internal company knowledge only**. Do NOT use for:

- General technology questions (use training knowledge)
...
- Access to Atlassian tools via the agent's tool integration
- A valid `cloudId` for the Atlassian instance
- Appropriate permissions to view target Confluence spaces and Jira projects
...
2. **Detailed Explanation** — Provide comprehensive details from all sources, organized by topic, not by source
3. **Source Attribution** — Note where each piece of information comes from (format: "According to [source], ...")
4. **Highlight Discrepancies** — If sources conflict, note it explicitly
5. **Provide Context** — Mention if information is outdated, deprecated, or in development
...
**Questionable results to watch for:**
...
- Assuming general knowledge applies to this company — always search first
- Making up information if search returns nothing
- Ignoring conflicting information between sources
- Quoting entire documents instead of summarizing
- Overwhelming with too many sources — curate top 5-10
- Forgetting to fetch details when snippets are insufficient

## Verification

After completing a search and synthesis, verify the quality of your answer:

1. **Check citations:** Every factual claim should have a source link
2. **Check completeness:** Did you fetch full content for the most relevant results?
3. **Check for conflicts:** Did you note any discrepancies between sources?
4. **Check freshness:** Note if key sources are more than 1 year old
5. **Check gaps:** Explicitly mention what you could NOT find

**Quick Reference:**

| Check | Command/Action |
|-------|----------------|
| Cross-system search executed | `search(cloudId, query)` called first |
| Targeted follow-up if needed | `searchConfluenceUsingCql` or `searchJiraIssuesUsingJql` |
| Full content fetched | `getConfluencePage` or `getJiraIssue` for top results |
| Citations included | Links to all sources in answer |
| Discrepancies noted | Conflicts between sources explicitly called out |
| Gaps acknowledged | Missing information clearly stated |

## Related skills

- **web-search** — For external documentation and general technology questions
- **read-confluence-page** — For direct page retrieval when you already know the page ID
