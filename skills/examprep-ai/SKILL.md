...
**Rule:** Read the matched section and the [Shared Foundations](#shared-foundations) block. Skip everything else. Do not load all sections for a focused request.
...
**Step 6 — Offer next steps:** *"Would you like (a) Flashcards, (b) Predicted Exam Paper, or (c) Readiness Dashboard?"*
...
| Out-of-syllabus question detected | "This doesn't match your syllabus — skipping it. Want me to include it anyway?" |
...
| Difficulty order violated | Never reverse Easy → Medium → Hard. This is a hard rule. |
...
|-------|------|---------------|
| Syllabus compliance | Every note maps to a syllabus unit | Grep each note's topic against the syllabus; ≥ 70% match required |
| Difficulty order | Easy before Medium before Hard — never reversed | Scan output top-to-bottom; 🟩 must appear before 🟨 before 🟥 |
...
| Coverage tracker complete | Every syllabus unit appears in the tracker | Count units in syllabus vs. rows in tracker — must match |
| Probability scores present | Every ranked question has a Priority label | Scan table: each row must have 🔥 Must or ✅ Do |
