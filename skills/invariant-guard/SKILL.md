...
- The algorithm family must be chosen before applying this skill. If the algorithm is unclear, load **`lemmaly`** first.
- For mathematical, probabilistic, FFT, or geometry algorithms, load **`mathguard`**. Invariants for approximate algorithms include ε-bounds, not equality.
- Load `invariant-guard` *after* the algorithm has been chosen and *before* the loop body is written.
...
Before claiming the function is correct, verify the following checklist:
