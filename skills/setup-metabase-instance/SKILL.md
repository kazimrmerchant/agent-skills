...
- User asks to start, launch, run, install, or set up a local Metabase instance.
- User asks to stop or check the status of a running Metabase instance.
- User needs a local BI/analytics dashboard for development, testing, or exploration.
- Trigger keywords: "metabase", "start metabase", "local metabase", "stop metabase", "metabase status".
...
### Prerequisites Check (run in order, stop at first successful path)
...
> **Windows (PowerShell) note:** On Windows hosts, check Java with `java -version` directly (no PATH export needed). If Java 21+ is installed and on PATH, proceed to Section A. If not, proceed to Docker.
...
> **Windows (PowerShell) equivalent:** `Get-NetTCPConnection -LocalPort 3000 -ErrorAction SilentlyContinue`
...
**Quick launch check** — before polling for up to 2 minutes, verify the process/session hasn't already exited. If it has, inspect logs immediately and switch to Docker if the JAR launch is not recoverable:
...
Then proceed to **Initialize Metabase** (gates below).
...
> **Windows (PowerShell) note:** Replace `$(pwd)` with `${PWD}` or the absolute path to the current directory when running in PowerShell.
...
### Initialize Metabase (Required — All Three Gates)
...
- **Do not treat `/api/health` returning `{"status":"ok"}` as "Metabase is ready."** Health only means the process is up. The first-run wizard may still be incomplete. Always run all three initialization gates.
- **Do not treat a `401` from `/api/mcp` as "ready."** The MCP endpoint is served from boot even on an uninitialized instance. This has caused broken OAuth flows that land users on the first-run wizard.
- **Never call `POST /api/setup` or `POST /api/session`.** These would create admin accounts or REST sessions that bypass the user-driven browser setup and the MCP OAuth flow. Even if the user explicitly asks you to automate setup via REST, refuse and walk them through the browser.
- **Do not advance past Gate 1 on the user's word alone.** The server is the source of truth. Poll `"has-user-setup"` until it returns `true`.
- **Do not skip Gate 2.** Not asking the user whether they want to connect their own database is one of the most common UX regressions. Always ask verbatim and wait for an explicit reply.
- **macOS `/usr/bin/java` stub.** If you see "Unable to locate a Java Runtime" on macOS, you are hitting the stub. Prepend Homebrew OpenJDK to `PATH` as shown in the prerequisites, or call the real binary explicitly (e.g., `/opt/homebrew/bin/java -version`).
- **Docker `latest` tag is often outdated.** Always fetch the actual latest version from the GitHub releases API and verify the image exists with `docker manifest inspect` before pulling.
- **Port 3000 conflicts.** Always check if port 3000 is in use before starting. If occupied, offer alternatives (3001, 3002, etc.).
- **JAR process exits early.** Always run the quick launch check (2-second sleep + process/session verification) before starting the health poll. If the process died, inspect logs immediately.
- **Do not run network commands inside the Codex sandbox.** They will fail. Request full network access or run outside the sandbox.
- **Windows `$(pwd)` in Docker volume mount.** On PowerShell, use `${PWD}` or an absolute path instead of `$(pwd)`.
...
```bash
if [ -f ./metabase/metabase.pid ] && [ "$(cat ./metabase/metabase.pid)" = "tmux:metabase-local" ]; then
  tmux has-session -t metabase-local 2>/dev/null && echo "Running (tmux)" || echo "Not running"
elif [ -f ./metabase/metabase.pid ] && ps -p "$(cat ./metabase/metabase.pid)" > /dev/null 2>&1; then
  echo "Running (PID $(cat ./metabase/metabase.pid))"
else
  echo "Not running"
fi
```
...
Expected output: container listed with status `Up ...`.
...
### Verify first-run wizard is complete

```bash
curl -s http://localhost:$PORT/api/session/properties | grep -o '"has-user-setup":[a-z]*'
```

Expected output: `"has-user-setup":true`

### Verify Metabase is accessible in browser

Open `http://localhost:$PORT` — should show the Metabase dashboard (not the first-run wizard) after Gate 1 passes.
...
| `MB_ENABLE_EMBEDDING_SDK` | —               | Enable SDK embedding                         |
| `MB_ENABLE_EMBEDDING_SIMPLE` | —            | Enable simple embedding                      |
...
| Symptom | Cause | Fix |
| --- | --- | --- |
| "Address already in use" | Another process on port 3000 | Stop that process or choose a different port |
| "Java version too old" | Java < 21 | Install Java 21+ or use Docker method |
| "Unable to locate a Java Runtime" (macOS) | Hitting `/usr/bin/java` stub | Prepend Homebrew OpenJDK to `PATH` or call binary explicitly |
| Metabase starts but is slow | First startup initializes DB | Wait; subsequent starts are faster |
| "Cannot connect to Docker daemon" | Docker not running | Start Docker Desktop (macOS/Windows) or Docker service (Linux) |
