...
- User says "deploy my app", "deploy and give me the link", "push this live", or "create a preview deployment".
- User wants to publish a website or web app to a live URL.
- User asks for a preview build or a staging link.

## Prerequisites

- **Node.js / npm** installed (for `npm install -g vercel`).
- **Git** installed if the project has a git remote and you intend to push.
- **Vercel CLI** (`vercel`) installed and authenticated for the primary path. If not present, the skill will install it.
- **Windows (PowerShell) is the primary host.** Commands below are POSIX-style for broad compatibility; on PowerShell, replace `2>/dev/null` with `2>$null` and use backtick line continuation or single-line forms. Path separators in examples are `/`; PowerShell accepts both `/` and `\`.

## Procedure

### Step 1: Gather Project State

Run all four checks before deciding which method to use. On PowerShell, replace `2>/dev/null` with `2>$null`.
...
**HARD RULE — Do NOT** use `vercel project inspect`, `vercel ls`, or `vercel link` to detect state in an unlinked directory. Without a `.vercel/` config, they will interactively prompt (or with `--yes`, silently link as a side-effect). Only `vercel whoami` is safe to run anywhere.
...
#### Case A: Linked (`.vercel/` exists) + has git remote → Git Push
...
#### Case B: Linked (`.vercel/` exists) + no git remote → `vercel deploy`
...
Use `--no-wait` so the CLI returns immediately with the deployment URL instead of blocking until the build finishes. Then check the deployment status with:
...
#### Case C: Not linked + CLI is authenticated → Link first, then deploy
...
4. **Then deploy using the best available method:**
   - If a git remote exists → commit and push (see Case A).
   - If no git remote → `vercel deploy [path] -y --no-wait --scope <team-slug>`, then `vercel inspect <url>` to check status.

#### Case D: Not linked + CLI not authenticated → Install, auth, link, deploy
...
**HARD RULE — Escalated network access:** Only escalate the actual deploy command if sandboxing blocks the network call (`sandbox_permissions=require_escalated`). Do **not** escalate the `command -v vercel` check.
...
**HARD RULE — Do not** curl or fetch the deployed URL to verify it works. Just return the link.

## Pitfalls

- **Silent side-effect linking.** Running `vercel link` or `vercel ls` with `--yes` in an unlinked directory will silently create a project. Only run state-detection commands that are safe without a `.vercel/` config (`vercel whoami` is the only safe one).
- **`vercel link` without `--repo` mismatches by directory name.** If the local folder name differs from the Vercel project name, plain `vercel link` often fails or links to the wrong project. Prefer `vercel link --repo` whenever a git remote exists.
- **Blocking deploys.** Always pass `--no-wait` to `vercel deploy` so the CLI returns the URL immediately instead of blocking on a long build.
- **Pushing without approval.** Never `git push` without explicit user approval, even when the project is linked.
- **Production deploys by accident.** Always deploy as preview unless the user explicitly asks for production. Use `--prod` only on explicit request.
- **Network egress on claude.ai.** If deployment fails due to network restrictions, tell the user:
  ```
...
- **Escalated network access (Codex / sandboxed environments).** If deployment fails due to network issues (timeouts, DNS errors, connection resets) in a sandboxed environment, rerun with escalated permissions (`sandbox_permissions=require_escalated`). Do not escalate the initial CLI availability check — only escalate the actual deploy command. Example guidance:
  ...
  ```
- **CLI auth failure.** If `vercel login` or `vercel deploy` fails with authentication errors, fall back to the no-auth deploy script (claude.ai or Codex variant, depending on the environment).
- **PowerShell redirection.** `2>/dev/null` is not valid on PowerShell; use `2>$null`. `cat` works on PowerShell but `Get-Content` is native.
- **Excluded files in no-auth fallback.** The fallback script excludes `node_modules`, `.git`, and `.env`. If the build needs `.env` values, the CLI path (with proper env config) is required instead.

## Verification

- **CLI deploy:** Run `vercel inspect <deployment-url>` and confirm the status is `READY` (or report `BUILDING` / `ERROR` to the user).
- **Git push:** Run `vercel ls --format json` after a short delay (`sleep 5`) and confirm the latest deployment entry's `url` matches the expected project slug.
- **No-auth fallback:** Confirm the script output JSON contains both `previewUrl` and `claimUrl` fields and present both to the user.
