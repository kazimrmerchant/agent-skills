...
- Triaging which example matches a user's need (payments → `with-stripe`, auth → `with-clerk`, etc.).
...
Map the user's need to an example name (e.g. payments → `with-stripe`, auth → `with-clerk`). `./references/catalog.md` is a categorized snapshot for fast triage — **load it when you need to triage which example matches a user's request** — but it drifts, so confirm against the live list:
...
> Windows (PowerShell) note: `/tmp/` does not exist. Use a local scratch path such as `$env:TEMP\expo-ref\with-stripe` or `.\.scratch\with-stripe`. Ensure the scratch dir is outside the user's project and gitignored.
...
1. **Version-align — don't copy pinned versions.** Examples track the **latest** SDK, so their `package.json` pins won't match an older project. Add only the *missing* deps with `npx expo install <pkg>` (it resolves SDK-correct versions) instead of copying exact versions.
2. **Merge config, don't replace it.** Add only the `app.json`/`app.config.*` plugins and permissions the example introduces that the user lacks — keep their existing config block intact.
3. **Port the integration code** from the example into the user's app at the appropriate location.
4. **Recreate env vars** from the example's `.env` shape — it holds placeholders, never working secrets. Use `YOUR_KEY`-style placeholders; never commit live secrets.
...
- **Default branch is `master`,** not `main` (matters for raw URLs and sparse checkout). Using `main` will 404 on raw URLs and break sparse-checkout.
- **Don't scaffold an example on top of an existing project.** Inspiration mode means read-and-adapt by hand, never `create-expo --example` into a populated dir.
- **Don't copy pinned versions from the example's `package.json`.** They target the latest SDK; use `npx expo install <pkg>` to resolve SDK-correct versions for the user's project.
- **Don't replace the user's `app.json`/`app.config.*`.** Merge only the new plugins/permissions the example introduces.
- **Deprecated examples** are removed from the repo tree but still listed in `meta.json` under `deprecated` with a `message` pointing to the modern path. Don't recommend a deprecated example — follow the `message`.
- **Aliases** in `meta.json` map old names to a `destination`; use the destination when recommending.
- **One-level listings miss nested integration code** (e.g. Stripe's `app/api/` server routes). Always use the recursive tree listing.
- **`.env` in examples holds placeholders, not working secrets.** Never copy real secrets from an example; never commit live keys.

## Verification

- Confirm the example exists and is not deprecated/aliased:
  ```bash
  gh api repos/expo/examples/contents/meta.json --jq '.content' | base64 -d
  ```
- Confirm the example directory is present in the live tree:
  ```bash
  gh api repos/expo/examples/contents --jq '.[] | select(.type=="dir" and (.name|startswith(".")|not)) | .name'
  ```
- After adapting into the user's app, verify deps resolve against the user's SDK:
  ```bash
  npx expo install --check
  ```
- Verify config plugins are valid:
  ```bash
...
- Verify the app still starts:
  ...
  npx expo start
  ```
- Single-click deploy sanity check (launch URL format): `https://launch.expo.dev/?github=https://github.com/expo/examples/tree/master/<example>`
...
- Auth → `with-clerk`
- Maps → `with-maps`
- Styling (NativeWind) → see `expo-tailwind-setup` related skill

## References

- `./references/catalog.md` — categorized snapshot of the example library for fast triage. **Load this when you need to map a user's request to an example name**, then confirm against the live list above (the catalog drifts).
