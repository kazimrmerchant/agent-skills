...
- GitLab CI/CD → recommend `gitlab-ci-patterns`.
- CircleCI, Jenkins, or other CI platforms.
- Pure Docker image building without GitHub context → recommend `docker-expert`.
- Kubernetes deployment configuration → recommend `kubernetes-architect`.

---

## Prerequisites

- A GitHub repository with write access (or org admin for runner/OIDC setup).
- GitHub CLI (`gh`) installed and authenticated for repo-level operations.
- PowerShell 5.1+ on Windows host (primary development environment).
- `jq` installed for dynamic matrix generation and context debugging.
- For OIDC: cloud provider admin access (AWS IAM, GCP IAM, or Azure AD) to configure trust policies.

### Windows Host Notes

All local discovery commands below use PowerShell. Workflow YAML itself runs on GitHub-hosted or self-hosted runners and is OS-agnostic.

```powershell
# Verify tooling on Windows host
gh --version
jq --version
```
...
### Step 1: Gather Context Before Responding

When invoked, first discover existing workflows, composite actions, and tech stack:

```powershell
# Discover existing workflows in the repo
Get-ChildItem -Path .github/workflows -Filter *.yml -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name
Get-ChildItem -Path .github/workflows -Filter *.yaml -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Name

# Check for composite actions
Get-ChildItem -Path .github/actions -Filter action.yml -Recurse -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName
...
Every production workflow must declare `permissions`, `concurrency`, `timeout-minutes`, and pin all actions to full commit SHAs.

```yaml
name: Workflow Name

on:                          # Triggers (see Step 3)
  push:
    branches: [main]
...
> **HARD RULE — Security Warning:** `pull_request_target` runs with repo secrets. Only use after a maintainer labels the PR. Never check out fork code without explicit sandboxing.
...
### Step 7: OIDC Authentication (Keyless Cloud Auth)

> **HARD RULE:** Never store long-lived cloud credentials as secrets. Use OIDC to get short-lived tokens that expire automatically.
...
### Step 8: Environments & Deployment Protection
...
### Step 12: Conditional Execution & Flow Control
...
> **HARD RULE:** Never place `${{ ... }}` directly inside `run:` when the value can come from PR metadata, workflow inputs, repository files, matrix JSON, or earlier job outputs. Put it in `env:` first, validate allowlisted values where possible, and reference the shell variable with quotes.
...
### Step 15: Release Automation Patterns

**Pattern 1: Build → Push → Deploy Pipeline:**
...
### Step 16: GitHub Actions Expressions Reference
...
| Script injection via `${{ }}` in `run:` | Attacker-controlled value expanded by shell | Pass through `env:` first, reference as shell variable |
| `pull_request_target` runs untrusted code | Fork PR checked out with repo secrets | Gate with label + `author_association` check; never checkout fork code without sandboxing |
| Actions pinned to mutable tags | `@v4` tag can be hijacked | Pin to full 40-char commit SHA |
| Windows runner line-ending issues | `echo "key=value" >> $GITHUB_OUTPUT` adds `\r` | Use `printf` or set `shell: bash` on Windows runners |
...
```powershell
# Install actionlint for workflow linting
# (requires Go or download binary from GitHub releases)
...
```powershell
# Verify all actions are pinned to full commit SHAs (40 hex chars)
Select-String -Path .github/workflows/*.yml -Pattern 'uses:\s+\S+@v\d' |
  ForEach-Object { Write-Host "UNPINNED: $($_.Line)" -ForegroundColor Red }

# Should output nothing if all actions are SHA-pinned
```

### Permissions Audit

```powershell
# Check that every workflow declares permissions
Get-ChildItem .github/workflows/*.yml | ForEach-Object {
    $content = Get-Content $_.FullName -Raw
    if ($content -notmatch 'permissions:') {
        Write-Host "MISSING permissions: $($_.Name)" -ForegroundColor Red
    }
}
```
...
```powershell
# Check that every job has timeout-minutes
Get-ChildItem .github/workflows/*.yml | ForEach-Object {
    $content = Get-Content $_.FullName -Raw
    if ($content -match 'jobs:' -and $content -notmatch 'timeout-minutes:') {
        Write-Host "MISSING timeout-minutes: $($_.Name)" -ForegroundColor Red
    }
}
```
