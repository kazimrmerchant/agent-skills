...
- Use when setting up authentication with Convex Auth or third-party providers (Clerk, Auth0, Better Auth, etc.)
- Use when working with Convex file storage, scheduled functions, or cron jobs
- Use when deploying or managing Convex projects
- Use when troubleshooting Convex function errors, schema push failures, or reactivity issues

## Prerequisites

- Node.js 18+ installed on your system
- A Convex account (free tier available) — sign in via GitHub during `npx convex dev`
- For Windows hosts: PowerShell is the primary shell. All `npx` commands work in PowerShell as-is
- TypeScript knowledge recommended for backend function authoring
- React/Next.js knowledge recommended for client-side integration
...
> **HARD RULE:** Never edit files in `convex/_generated/`. These are auto-generated and overwritten on every `npx convex dev` sync.
...
> **HARD RULE:** In compound indexes, put equality filters first and range filters last. The order of fields in the index definition determines which query patterns it can efficiently serve.
...
> **HARD RULE:** Actions cannot directly access the database — they must use `ctx.runQuery` and `ctx.runMutation`. Never access `ctx.db` directly in actions.
...
npx convex env set SENDGRID_API_KEY YOUR_KEY
...
> **HARD RULE:** Environment variables are only accessible in **actions** (not queries or mutations) because queries/mutations run in the deterministic transaction engine.

```typescript
// Only available in actions — NOT in queries or mutations
...
6. **❌ Storing large blobs in documents**: Use Convex file storage (`_storage`) for files; keep documents lean. Document size limit is 1MB.
7. **❌ Circular `runQuery`/`runMutation` chains**: Actions calling mutations that schedule actions can create infinite loops.
8. **❌ Editing `convex/_generated/` files**: These are auto-generated and overwritten on every sync.
...
```powershell
# Check that convex is installed
npm ls convex

# Verify .env.local was created with required variables
type .env.local
# Should contain:
# CONVEX_DEPLOYMENT=...
# NEXT_PUBLIC_CONVEX_URL=...

# Verify convex folder structure exists
dir convex\_generated\api.d.ts
dir convex\schema.ts
```

### Verify Schema and Functions Sync

```powershell
# Start dev mode — should sync without errors
npx convex dev

# In a separate terminal, run a function to verify it works
npx convex run tasks:list
...
```powershell
# List all env vars — confirm your keys are set
npx convex env list
```
...
# Verify deployment by running a function against prod
...
```powershell
# Confirm auth packages installed
