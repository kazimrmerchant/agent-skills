...
Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI). Claude Code v2.x reads files, writes code, runs shell commands, spawns subagents, and manages git workflows autonomously.

**Windows host is primary (PowerShell).** When commands use bash syntax (e.g., `cat file | claude -p`), adapt for PowerShell (e.g., `Get-Content file | claude -p`). tmux commands apply when running inside WSL or a Linux/macOS environment.

## When to Use

- One-shot coding tasks: fix a bug, add a feature, refactor a module
- CI/CD automation and scripting with structured JSON output
- Multi-turn iterative work: refactor → review → fix → test cycles
- Code review and security analysis of current changes
- Structured data extraction from codebases using `--json-schema`
- Piped input processing (pipe file contents or command output for analysis)
- Session continuation and resumption across directories

## Prerequisites

1. **Install:** `npm install -g @anthropic-ai/claude-code`
2. **Auth (browser OAuth for Pro/Max):** run `claude` once to log in
3. **API key auth:** set `ANTHROPIC_API_KEY` environment variable
4. **Console auth (API key billing):** `claude auth login --console`
5. **SSO auth (Enterprise):** `claude auth login --sso`
6. **Check status:** `claude auth status` (JSON) or `claude auth status --text` (human-readable)
7. **Health check:** `claude doctor` — checks auto-updater and installation health
8. **Version check:** `claude --version` (requires v2.x+)
9. **Update:** `claude update` or `claude upgrade`
...
```powershell
Get-Content src/auth.py | claude -p "Review this code for bugs" --max-turns 1
Get-Content src/*.py | claude -p "Find all TODO comments" --max-turns 1
git diff HEAD~3 | claude -p "Summarize these changes" --max-turns 1
```
...
```bash
# Start a task and save session info
...
# Resume the most recent session in the same directory
...
# Wait for startup, then send your task (after ~3-5 seconds for the welcome screen)
...
| Key | Action |
|-----|--------|
| `Shift+Tab` | Cycle auto-accept / plan / default mode |
| `Tab` | Toggle auto-accept edits mode |
...
- **`scripts/claude-cc`** — Wrapper script that unsets `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_API_KEY` before `exec claude "$@"`, forcing Claude Code to fall back to browser OAuth. Use this wrapper whenever Claude Code runs as a sub-agent inside Hermes and the host's active provider is not Anthropic. Verify with: `bash scripts/claude-cc -p "say PONG" --max-turns 1`.
- Load reference files from `references/` when you need deeper context on specific topics. Check the directory for available reference documents.

## Pitfalls

1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
...
### CRITICAL: Injected ANTHROPIC_AUTH_TOKEN breaks OAuth

When Claude Code runs **as a sub-agent inside Hermes**, Hermes injects `ANTHROPIC_AUTH_TOKEN` into the subprocess environment — sourced from the host's model config `api_key` (e.g. the Ollama provider sets `api_key: ollama`, which Hermes exports as `ANTHROPIC_AUTH_TOKEN=ollama`). Claude Code treats that as a bearer and sends it → **401 Invalid bearer token**, even though `claude auth status` reports `loggedIn: true` (it only checks local credential presence, not validity). `env -u ANTHROPIC_API_KEY` alone does NOT fix it — the offending var is `ANTHROPIC_AUTH_TOKEN`.

**Fix:** run Claude Code via the wrapper `scripts/claude-cc`, which unsets `ANTHROPIC_AUTH_TOKEN` and `ANTHROPIC_API_KEY` before `exec claude "$@"`, forcing Claude Code to fall back to browser OAuth (your claude.ai membership). Verify with: `bash scripts/claude-cc -p "say PONG" --max-turns 1`.

**Scope note:** this applies whenever Hermes' active/default provider is not Anthropic but a stale/placeholder Anthropic token leaks into the env. The claude.ai membership (OAuth) can power Claude Code as a *sub-agent*; it **cannot** be Hermes' *own* model — that still requires a real `ANTHROPIC_API_KEY` in Hermes' `.env` + `model.provider=anthropic`.
...
1. **Check installation health:**
   ...
   claude doctor
   ```
   Should report no issues with auto-updater and installation.

2. **Verify version (requires v2.x+):**
   ```bash
   claude --version
   ```
...
4. **Verify print mode works:**
   ```bash
   claude -p "say PONG" --max-turns 1
   ```
   Should output "PONG" and exit.

5. **Verify wrapper (if using scripts/claude-cc):**
   ```bash
   bash scripts/claude-cc -p "say PONG" --max-turns 1
   ```
   Should output "PONG" and exit — confirms OAuth fallback works.

6. **Verify JSON output format:**
   ```bash
   claude -p "What is 2+2" --output-format json --max-turns 1
   ```
   Should return a JSON object with `"subtype": "success"`.

7. **Verify tmux session (interactive mode):**
   ```bash
   tmux new-session -d -s test-claude -x 140 -y 40
   tmux send-keys -t test-claude "claude -p 'say PONG' --max-turns 1" Enter
   sleep 5 && tmux capture-pane -t test-claude -p -S -10
   tmux kill-session -t test-claude
   ```

## Related Skills

- **codex** — Alternative coding agent CLI
- **hermes-agent** — Hermes orchestration framework
