...
- Automating `tmux` commands from bash scripts without user interaction
...
- Optional: `~/.tmux.conf` under version control for reproducible configs
...
### 4. Sending Commands to Panes Without Being Attached
...
### 5. Scripting a Full Workspace Layout

Create a fully configured multi-pane workspace from a single script.
...
- **`tmux` commands from a script fail with "no server running"** — Start the server first with `tmux start-server`, or create a detached session before running other commands.
- **Pane size is 0×0 when creating a detached session** — Pass explicit dimensions: `tmux new-session -d -s name -x 200 -y 50`.
- **`send-keys` types the text but doesn't run the command** — Ensure you pass `Enter` (capital E) as a second argument: `tmux send-keys -t target "cmd" Enter`.
- **Script creates a duplicate session each run** — Guard with `tmux has-session -t name 2>/dev/null || tmux new-session -d -s name`.
- **Copy-mode selection doesn't work as expected** — Confirm `mode-keys vi` or `mode-keys emacs` is set to match your preference in `~/.tmux.conf`.
- **Wrong pane receives keystrokes** — Always specify the full target `session:window.pane`; never rely on "current pane" from outside an attached client.

## Verification

```bash
# 1. Confirm tmux is installed and show version
tmux -V
# Expected: tmux 3.x

# 2. Create a detached test session with explicit size
tmux new-session -d -s verify -x 200 -y 50

# 3. Confirm it exists
tmux has-session -t verify 2>/dev/null && echo "exists"
# Expected: exists

# 4. Send a command and capture output
tmux send-keys -t verify:0 "echo hello-from-tmux" Enter
sleep 1
tmux capture-pane -t verify:0 -p | grep "hello-from-tmux"
# Expected: hello-from-tmux

# 5. List sessions
tmux ls
# Expected: verify: 1 windows (created ...) [200x50]
