...
## Overview
Spatial Design creates UI that belongs in the room with you. It features transparent, glass-like panels that react to the lighting of the physical space.

## When to Use
Use this skill when the user's request matches the spatial design aesthetic: environment-aware layouts, Apple Vision Pro inspiration, and mixed reality aesthetics. This is a child reference of the `design-it` skill.

## Prerequisites
- A complex background image or environment simulation is required for the glass effect to look right.
- Platform-specific blur libraries may be required (e.g., `@react-native-community/blur` for React Native).

## Procedure

### 1. Core Principles
1. **Environmental Transparency**: The UI acts like a glass pane. It relies on background blur to let the environment dictate the mood.
2. **Dynamic Lighting**: Elements respond to cursor position as if a flashlight is shining on them.
3. **Subtle Volume**: Elements have a very thin rim of light around the edge (specular highlight).

### 2. Visual DNA
- **Colors**: Almost exclusively uses `rgba()` white or black. The actual color comes entirely from the background environment.
- **Typography**: Extremely sharp, high legibility. Often uses varying font weights to establish hierarchy without relying on color. Apple's `SF Pro` is the gold standard.
- **Icons**: Outlined, high-legibility glyphs.

### 3. Web Implementation (CSS)
```css
body {
...
- **Limitations**: This is a styling reference and does not replace environment-specific validation, accessibility testing, or expert review. Ensure appropriate contrast ratios and responsive behaviors are verified separately.

## Verification
1. **Visual Check**: Ensure the background environment is visible but blurred through the UI panels.
2. **Specular Highlight**: Verify a thin rim of light is present around the edges of the glass elements.
3. **Contrast**: Check that text remains highly legible against the blurred background. Use varying font weights for hierarchy.
4. **Shadows**: Confirm shadows are soft and diffuse, not dark or opaque.
