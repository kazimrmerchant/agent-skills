...
- Access to the provider dashboard for every paid API you intend to call (Fal.ai, Anthropic, OpenAI, Replicate, ElevenLabs, Together, Groq, Cohere, Mistral, etc.).
- Knowledge of the unit cost for each model / endpoint you plan to call (check provider pricing pages).
- A queue / job framework configured if using Inngest, BullMQ, Sidekiq, or Cloud Tasks (for concurrency and retry settings).
- `p-limit` (npm) or equivalent concurrency-limiter installed if doing in-process fan-out in Node.js / TypeScript.
- Per-environment API keys (dev / staging / prod) — never share a single key across environments.
...
### Step 1 — State the cost contract (before writing code)
...
3. **Max $ per run.** `max_calls × unit_cost`. Compute it, don't estimate.
...
7. **Amplifier audit.** Walk the amplifier list in Step 5 below; declare "none apply" or address each that does.
...
### Step 2 — Set the provider-side hard cap (out of band, before first request)
...
### Step 3 — Write the code with explicit bounds

Every loop calling a paid API gets an explicit iteration bound — a **concrete integer in code**, not just "eventually terminates":
...
Every call site gets a one-line cost contract comment:
...
### Step 4 — Bound retries and concurrency
...
- Inngest default is **4 retries** (up to 5 attempts including the initial — confirm against current Inngest docs). Worst-case wallet math: `attempts = 1 + retries`, so a default `step.run()` can bill **5×**, not 4×.

**Concurrency limit:**
- State the limit in code, at the queue (Inngest `concurrency`), and at the provider where supported.
...
**Idempotency keys:**
- Every mutating or charging call gets an idempotency key. A webhook that fires twice should bill once. Without an idempotency key, retry policies you cannot see (load balancer, framework, gateway) silently double-charge.

### Step 5 — Amplifier audit

Walk this list before shipping. Declare "none apply" or address each that does:
...
Enumerate these scenarios before shipping:
...
**OpenAI:** Per-project monthly budgets are **soft only** — they alert but do not block. A runaway can continue past the documented project budget. For a real hard cap, use org-level Usage limits, a billing gateway/proxy, or your own fail-closed budget check in code.

**Inngest:** Default is **4 retries** (up to 5 attempts including the initial — confirm against current Inngest docs). Worst-case wallet math: `attempts = 1 + retries`, so a default `step.run()` can bill **5×**, not 4×. Always set `retries: 2` or lower for paid call functions.

**Anthropic:** Prompt caching reduces cost ~90% for repeated context; the Workspace Budget cap is on unblended cost so caching extends the budget — but do not assume caching is always active.

**Fal.ai:** Webhook deliveries are paid; cap retries on your side, not just on the generation call.

**ElevenLabs:** Voice cloning is billed per character; cap the character count in code AND per key.
...
Before shipping any code that calls a paid API, check every box:
...
- [ ] Amplifier audit (Step 5) has been performed and either "none apply" or each addressed.
...
```powershell
# Check that no paid API key is shared across environments (Windows / PowerShell)
Select-String -Path .\.env* -Pattern "API_KEY|SECRET|TOKEN" | Select-Object Filename, LineNumber, Line

# Verify p-limit is installed if doing in-process fan-out
npm ls p-limit

# Grep for unbounded Promise.all over paid APIs (red flag)
Select-String -Path .\src\*.ts -Pattern "Promise\.all\(.*\.map\(.*paidApi"
```
