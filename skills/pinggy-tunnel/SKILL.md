...
### 4. Verify the tunnel end-to-end
...
- **Windows PowerShell quoting.** On Windows (PowerShell), the `+` in the username string is fine inside double quotes, but `$PINGGY_TOKEN` must use PowerShell syntax (`$env:PINGGY_TOKEN`). Use `Start-Process ssh -ArgumentList ...` for background tunnels, or run in a separate terminal window. The `nohup` / `&` background pattern is bash-only; on PowerShell use `Start-Job` or `Start-Process`.
...
### Example 1 — Receive a webhook callback
...
### Example 2 — Expose an MCP server over HTTP/SSE
...
### Example 3 — Expose a local LLM endpoint (Ollama / vLLM / llama.cpp)
...
### Example 4 — Share a dev server with a one-shot password
...
- **cloudflared-quick-tunnel** — prefer when `cloudflared` is already installed; Cloudflare quick tunnels have no 60-minute expiry.
- **webhook-subscriptions** — for managing persistent webhook endpoint registrations that pair with a Pinggy tunnel for local receipt.
