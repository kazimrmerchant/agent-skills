...
- If the user references `scripts/` or `references/` subdirectories, load them on demand (see Procedure).
...
- If the user references `scripts/` — load relevant scripts from the `scripts/` directory within this skill folder when needed for examples or diagnostics.
- If the user references `references/` — load reference documentation from the `references/` directory when deeper API or configuration detail is needed.
...
- **Installing `libcuopt` alone expecting Python API** — it does NOT include the Python API; install `cuopt` instead
...
1. **Solver status** is `Optimal`, `FeasibleFound`, or `SUCCESS`
2. **All constraints** are satisfied
3. **Objective value** is reasonable for the problem size and type
