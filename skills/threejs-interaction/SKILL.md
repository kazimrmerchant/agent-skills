...
- Basic understanding of Three.js scene, camera, and renderer setup (see `threejs-fundamentals`).
...
### 7. World-Screen Coordinate Conversion
...
## Pitfalls
- **OrbitControls Damping**: If `enableDamping` is true, you MUST call `controls.update()` in the animation loop.
- **Mouse Coordinates**: Always convert `clientX/clientY` to Normalized Device Coordinates (NDC) (-1 to 1) before raycasting. Failing to invert the Y-axis (`-(clientY / height) * 2 + 1`) will cause inverted picking.
- **Canvas Offset**: If the canvas is not full screen, use `getBoundingClientRect()` to calculate correct NDC.
- **TransformControls Conflict**: When using TransformControls with OrbitControls, disable OrbitControls during `dragging-changed` events to prevent camera movement while dragging the gizmo.
- **Performance**: Raycasting on every `mousemove` is expensive. Throttle hover raycasts (e.g., 20fps max) or use simpler invisible collision meshes.
- **Disposal**: Always remove event listeners when destroying the scene/interaction manager to prevent memory leaks.

## Verification
1. **Check OrbitControls**: Verify that the camera rotates when dragging the mouse, and zooms with the scroll wheel.
2. **Check Raycasting**: Click on a 3D object and verify the console logs the correct object or the object changes color.
3. **Check Touch Input**: On a touch device, tap an object and verify it registers as a click/selection.
4. **Check Coordinate Conversion**: Place an HTML element at a 3D object's position using `worldToScreen` and verify it stays attached when the camera moves.
