...
- Trigger keywords: "review Django performance", "find N+1 queries", "optimize Django", "check queryset performance", "database performance", "Django ORM issues", "audit Django code for performance".

## Prerequisites

- A Django or Django REST Framework codebase accessible via `Read`, `Grep`, and `Glob`.
- Sufficient context to trace data flow from view/queryset creation to template/serializer consumption.
- Ability to run `grep`/`Grep` searches across the codebase to validate findings.

## Procedure

### Step 0: Adopt the Review Mindset
...
### Step 1: Triage by Impact Category
...
### Step 2: Check for N+1 Queries (CRITICAL)
...
### Step 3: Check for Unbounded Querysets (CRITICAL)
...
### Step 4: Check for Missing Indexes (HIGH)
...
### Step 5: Check for Write Loops (HIGH)
...
### Step 6: Check for Inefficient Patterns (LOW)
...
### Step 7: Validate Every Finding Before Reporting
...
The biggest risk in Django performance review is manufacturing findings to appear thorough. Zero validated findings is a valid and professional outcome. Do not inflate severity to make a finding seem important.

## Verification

To verify the review was conducted correctly:

1. **Every reported finding has a Validation section** with traced data flow, codebase search results, data volume confirmation, and hot-path verification.
2. **No finding lacks evidence** — if a finding has no code snippet or traced path, it should not be in the report.
3. **Severity matches impact** — CRITICAL findings involve loops multiplying queries or unbounded memory; HIGH involves large-table scans or write loops; LOW is reserved for minor notes.
4. **Excluded categories are absent** — no findings from test files, migrations, admin-only views, management commands, or tables with <1000 rows.
5. **False positives are absent** — no reports of queryset variable assignment, single-query patterns, or single-object multi-fetch as N+1.
6. **Zero findings is valid** — if no validated issues exist, the report states this explicitly with a summary of what was checked.

### Quick Grep Commands for Validation

Use these to confirm findings during review:

```powershell
# Find existing prefetch/select_related optimizations
grep -rn "select_related\|prefetch_related" --include="*.py"

# Find unbounded queryset patterns
grep -rn "\.all()" --include="*.py" | grep -v "test"

# Find pagination configuration
grep -rn "paginate_by\|PaginationClass\|pagination_class" --include="*.py"

# Find bulk operations
grep -rn "bulk_create\|bulk_update\|\.update(" --include="*.py"

# Find write loops
grep -rn "\.save()\|\.create()\|\.delete()" --include="*.py" | grep -v "test"
```
