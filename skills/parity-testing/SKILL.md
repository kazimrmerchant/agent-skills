...
## When to Use
Use this skill when debugging or verifying numerical parity between pipeline implementations (e.g., research repo vs diffusers, standard vs modular). It is also highly relevant when pipeline outputs look wrong — washed out, pixelated, or have visual artifacts — as these are usually parity bugs.

## Prerequisites
1. Identify which two implementations are being compared (e.g., research repo → diffusers, standard → modular, or research → modular). Use `AskUserQuestion` with structured choices if not already clear.
2. Ensure you have two equivalent runnable scripts — one for each implementation, both expected to produce identical output given the same inputs. These scripts define what "parity" means concretely.
3. If invoked from the `model-integration` skill, the reference script comes from step 2 of setup, and the diffusers script is the one you just wrote. Ensure both scripts are runnable and use the same inputs/seed/params.

## Procedure

### 1. Component Parity (CPU/float32) - Always run as you build
Test each component before assembling the pipeline. Each component in isolation, strict `max_diff < 1e-3`.
...
### 2. E2E Visual (GPU/bfloat16) - Once the pipeline is assembled
...
### 3. Pipeline Stage Tests - Only if E2E fails
...
### 4. Debugging Technique: Injection for root-cause isolation
...
## Verification
Use the following comparison utility to verify tensor parity between implementations:
...
To verify component parity, run your component test scripts and ensure the assertion `max_diff < 1e-3` passes for both `fresh` and `saved` modes.
To verify E2E parity, visually inspect the generated outputs (or extracted frames for video pipelines) to ensure they are identical.
