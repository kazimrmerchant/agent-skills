...
**When NOT to use:**
- Linux or Windows users — this skill covers macOS-specific paths and tools (`~/Library/Application Support/`, `osascript`, `$TMPDIR`)
...
- macOS host (this skill is macOS-only; paths and commands use `osascript`, `getconf DARWIN_USER_TEMP_DIR`, and `~/Library/Application Support/`)
...
1. **MCP connects at startup only.** If the browser bridge wasn't ready when Claude Code started, the connection will fail for the entire session. The fix is: ensure Chrome + extension are running with correct config, THEN restart Claude Code. Do not skip the Claude Code restart.

2. **Multiple Chrome Profiles.** If the Claude extension is installed in multiple Chrome profiles, each spawns its own native host and socket, causing confusion. Fix: Only enable the Claude extension in ONE Chrome profile.

3. **Multiple Claude Code Sessions.** Running multiple Claude Code instances causes socket conflicts. Fix: Only run one Claude Code session at a time, or use `/mcp` to reconnect after closing other sessions.

4. **Hardcoded Version in Wrapper.** The wrapper at `~/.claude/chrome/chrome-native-host` may have a hardcoded version that becomes stale after updates.
   ```bash
...
5. **TMPDIR Not Set.** Claude Code expects `TMPDIR` to be set to find the socket.
   ```bash
   # Check
   echo $TMPDIR
...
6. **Cannot use both hosts simultaneously.** Claude.app (Cowork) and Claude Code CLI native hosts are mutually exclusive. You must disable one for the other to work.
...
```bash
# 1. Verify correct native host is running (Claude Code CLI)
ps aux | grep chrome-native-host | grep -v grep
# Expected: ~/.local/share/claude/versions/X.X.X --chrome-native-host

# 2. Verify socket exists (Claude Code CLI)
ls -la "$(getconf DARWIN_USER_TEMP_DIR)/claude-mcp-browser-bridge-$USER"
# Expected: socket file exists and is not stale

# 3. Verify only one config is active
ls ~/Library/Application\ Support/Google/Chrome/NativeMessagingHosts/com.anthropic*.json
# Expected: only one .json file (the other should be .json.disabled)

# 4. Verify TMPDIR is set
echo $TMPDIR
# Expected: /var/folders/XX/.../T/

# 5. Test MCP tool from Claude Code
# Call any mcp__claude-in-chrome__* tool — should return successfully
# instead of "Browser extension is not connected"
```
...
- Any skill that uses `mcp__claude-in-chrome__*` tools for browser automation may trigger this troubleshooting skill when those tools fail.
