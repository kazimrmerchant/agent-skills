---
name: peft
description: "Fine-tunes LLMs with Hugging Face PEFT (LoRA, QLoRA, IA3, multi-adapter serve) training under 1% of weights. Use when adapting 7B–70B models on limited GPU memory or serving multiple adapters from one base. Not for full-weight fine-tuning of small models and not for hosting Gradio demos (huggingface-spaces)."
version: 1.0.1
...
### 4. Loading and Merging Adapters
...
1. **Verify installation and versions:**
...
2. **Verify trainable parameters after applying LoRA:**

```python
model.print_trainable_parameters()
# Expected: trainable params: 13,631,488 || all params: 8,043,307,008 || trainable%: 0.17%
```

3. **Verify adapter is active:**

```python
print(model.active_adapters)  # Should show adapter name, e.g., ['default']
```

4. **Verify saved adapter files:**

```powershell
# Check adapter directory contains adapter_config.json and adapter_model.safetensors
Get-ChildItem ./lora-llama-adapter
```

Expected files: `adapter_config.json`, `adapter_model.safetensors` (or `adapter_model.bin`).

5. **Verify merged model loads correctly:**

```python
merged_model = model.merge_and_unload()
# Should run without error and produce a standard transformers model
