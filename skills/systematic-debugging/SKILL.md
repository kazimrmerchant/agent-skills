...
- Access to the project's test runner (pytest, jest, go test, etc.)
- Git access for history inspection
- Ability to run terminal commands in the project root
- On Windows hosts, commands below show PowerShell equivalents where they differ from bash

## Procedure

You MUST complete each phase before proceeding to the next. There are no shortcuts.

---

### Phase 1: Root Cause Investigation

**BEFORE attempting ANY fix, complete every step below.**
...
PowerShell equivalent for the flaky loop:

```powershell
1..100 | ForEach-Object { pytest tests/test_flake.py::test_name -q; if (-not $?) { break } }
```
...
#### Step 0 — Minimize the Reproduction
...
**Scientific method — no guessing.**

#### Step 1 — Form Ranked Falsifiable Hypotheses
...
#### Step 4 — If Fix Doesn't Work: The Rule of Three
...
**If 3+ fixes failed:** Question the architecture (Phase 4, Step 5). Do not attempt another fix.
...
- **Skipping the tight loop on flaky bugs:** A 1% flake is not debuggable without raising the reproduction rate. Spend disproportionate effort here.
- **Bundling fixes:** Changing two things at once means you cannot know which one worked. One variable at a time, always.
- **Fixing at the symptom site:** If a bad value originates three calls upstream, fixing the downstream check masks the real bug and creates future surprises.
- **Not cleaning up debug logs:** Tag every temporary log line with a unique prefix (e.g., `[DEBUG-a4f2]`) so a single search removes them all.
- **Ignoring environment differences:** Windows vs Linux path separators, line endings, env var casing, and shell quoting differences can cause bugs that only reproduce on one platform.
- **Trusting "that can't matter":** Small differences between working and broken code are the most likely root cause. List every difference, however small.

## Verification

After completing Phase 4, verify the fix is real and complete:

### 1. Regression Test Passes

```bash
# The specific test that was red in Phase 1 must now be green
pytest tests/test_module.py::test_regression -v
```
...
**Expected output:** test passes with `PASSED` status.

### 2. Full Test Suite — No New Failures

```bash
pytest tests/ -q
```

**Expected output:** no new failures compared to the pre-fix baseline.

### 3. Debug Artifacts Removed

```bash
# Verify no temporary debug logs remain
grep -rn "\[DEBUG-" src/ tests/
```

```powershell
# PowerShell
Select-String -Path "src\*.py","tests\*.py" -Pattern "\[DEBUG-"
```

**Expected output:** no matches. All temporary instrumentation cleaned up.

### 4. Minimal Reproduction Is Now a Regression Test

Confirm the minimized repro from Phase 2 Step 0 has been committed as a permanent regression test.

### 5. Root Cause Is Documented

The commit message or PR description states the root cause, not just the symptom that was fixed.

## Examples

### With delegate_task — Multi-Component Investigation
...
### With test-driven-development — Bug Fix Workflow

When fixing bugs:
1. Write a test that reproduces the bug (RED)
2. Debug systematically to find root cause (Phase 1–3)
...
- **test-driven-development** — Write the failing test first, then fix root cause
- **plan** — Use when the fix requires architectural changes (Phase 4, Step 5)
- **subagent-driven-development** — Dispatch investigation subagents for multi-component systems
...
| **2. Pattern** | Minimize repro, find working examples, compare, identify differences | Know what's different |
| **3. Hypothesis** | Form ranked theory, test minimally, one variable at a time | Confirmed or new hypothesis |
| **4. Implementation** | Create regression test, fix root cause, verify, Rule of Three | Bug resolved, all tests pass |
